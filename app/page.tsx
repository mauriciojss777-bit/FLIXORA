...
function SocialBarScript({ src }: { src: string }) {
  useEffect(() => {
      ...
      <div className="w-full max-w-[100vw] overflow-x-hidden">
        ...
        <nav className="sticky top-0 z-40 bg-[#0f0f0f]/95 backdrop-blur-xl border-b border-zinc-800 px-4 py-3 flex items-center justify-between gap-3 w-full max-w-[100vw]">
          <div className="flex items-center gap-1.5 flex-shrink-0">
            ...
            <button onClick={() => setShowSearchModal(true)} className="text-zinc-200 hover:bg-zinc-800 p-2 rounded-xl transition-colors focus:outline-none flex items-center justify-center" aria-label="Buscar">
              <svg className="w-5 h-5 text-zinc-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
            </button>
          </div>
        ...
        {showSearchModal && (
          <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 px-4 bg-black/80 backdrop-blur-sm" onClick={() => setShowSearchModal(false)}>
            <div className="bg-[#141414] border border-zinc-800 p-4 rounded-2xl max-w-lg w-full space-y-3 shadow-2xl" onClick={e => e.stopPropagation()}>
              <div className="flex justify-between items-center pb-2 border-b border-zinc-800">
                <h3 className="text-xs font-bold text-white uppercase tracking-wider">Buscar contenido</h3>
                <button onClick={() => setShowSearchModal(false)} className="text-zinc-400 hover:text-white text-xs font-bold">✕</button>
              </div>
              <div className="relative">
                <input 
                  type="text" 
                  placeholder="Escribe título, categoría o etiqueta..." 
                  value={searchQuery} 
                  onChange={(e) => setSearchQuery(e.target.value)} 
                  autoFocus
                  className="w-full bg-zinc-900 border border-zinc-800 p-3 pl-10 rounded-xl text-xs focus:border-blue-500 outline-none text-zinc-200" 
                />
                <svg className="w-4 h-4 text-zinc-400 absolute left-3 top-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
              </div>
              <div className="flex justify-end pt-2">
                <button onClick={() => setShowSearchModal(false)} className="bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold px-4 py-2 rounded-xl">Buscar</button>
              </div>
            </div>
          </div>
        )}
