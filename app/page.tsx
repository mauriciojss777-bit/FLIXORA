...
function AdsterraBlock({ zoneId }: { zoneId: string }) {
  ...
  useEffect(() => {
    ...
    // @ts-ignore
    window.atOptions = { 'key': zoneId, 'format': 'iframe', 'height': 250, 'width': 300, 'params': {} };
...
function HorizontalVideoCard({
  ...
  const getSafeIframeHtml = () => {
    const isUrlIframe = video.voe_url.includes('<iframe');
    if (isUrlIframe) {
      return video.voe_url
    ...
    return `<iframe src="${video.voe_url}${separator}autoplay=1&mute=1&controls=0" class="w-full h-full border-0 pointer-events-none" sandbox="allow-scripts allow-same-origin allow-presentation" allow="autoplay" title="${video.title}"></iframe>`;
  };
          ...
          {!isHovered || video.is_photo ? (
            ...
            <div className="w-full h-full absolute inset-0 overflow-hidden flex items-center justify-center bg-black pointer-events-none">
              <div
                ...
                dangerouslySetInnerHTML={{ __html: getSafeIframeHtml() }}
              />
            </div>
          )}
          <button
            ...
            {selectedVideo.is_short ? (
              ...
              >
                {videos.filter(v => v.is_short).map((v, sIndex) => {
                  ...
                  const getShortEmbedHtml = () => {
                    const isUrlIframe = v.voe_url.includes('<iframe');
                    if (isUrlIframe) {
                      return v.voe_url.replace('<iframe', '<iframe sandbox="allow-scripts allow-same-origin allow-presentation"');
                    }
                    const separator = v.voe_url.includes('?') ? '&' : '?';
                    return `<iframe src="${v.voe_url}${separator}autoplay=1" class="w-full h-full border-0" sandbox="allow-scripts allow-same-origin allow-presentation" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen title="${v.title}"></iframe>`;
                  };
                      ...
                      <div className="w-full h-full max-w-md relative flex items-center justify-center bg-black">
                        {isCurrent ? (
                          <div
                            ...
                            dangerouslySetInnerHTML={{ __html: getShortEmbedHtml() }}
                          />
                        ) : (
