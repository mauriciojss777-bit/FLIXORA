    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-6 w-full overflow-x-hidden">
        <div className="bg-zinc-950 border border-zinc-800 p-8 rounded-3xl max-w-sm w-full text-center space-y-6">
          <h1 className="text-4xl font-black text-white tracking-tight">FLI<span className="text--500">XORA</span></h1>
          <p className="text-xs text-zinc-400">Este sitio contiene material para adultos. Debes ser mayor de edad para ingresar.</p>
          <button onClick={() => { localStorage.setItem('age_verified', 'true'); setAgeAccepted(true); }} className="w-full bg--500 hover:bg--400 text-black font-black py-4 rounded-xl transition-colors">INGRESAR</button>
        </div>
      </div>
    );
  }
    ...
    <main className={`min-h-screen ${isCinemaMode ? 'bg-black' : 'bg-[#0f0f0f]'} text-zinc-200 flex flex-col justify-between w-full max-w-full overflow-x-hidden transition-colors duration-300 box-border`}>
      <div className="w-full max-w-full overflow-x-hidden box-border">
        <nav className="sticky top-0 z-40 bg-[#0f0f0f]/95 backdrop-blur-xl border-b border-zinc-800 px-3 py-3 flex items-center justify-between gap-2 w-full max-w-full box-border">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <h1 className="text-xl sm:text-2xl font-black text-white cursor-pointer tracking-tight truncate" onClick={() => { setActiveTag('Todos'); setSearchQuery(''); handleCloseVideo(); }}>
              FLI<span className="text--500">XES</span>
            </h1>
          </div>
          ...
          <div className="hidden sm:flex items-center flex-1 max-w-xl mx-4">
            <div className="relative w-full">
              <input 
                ...
                className="w-full bg-[#141414] border border-zinc-700 pl-4 pr-10 py-2 rounded-full text-sm text-zinc-200 focus:border--500 outline-none" 
              />
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <button onClick={() => setShowWatchLaterModal(true)} className="bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-[11px] px-2.5 py-1.5 rounded-full font-bold border border-zinc-700 transition-all whitespace-nowrap">
              ⭐ Guardados {watchLater.length > 0 && <span className="ml-0.5 bg--500 text-black px-1.5 py-0.2 rounded-full text-[9px] font-black">{watchLater.length}</span>}
            </button>
            <button onClick={() => setShowDonateModal(true)} className="bg--500 hover:bg--400 text-black text-[11px] px-2.5 py-1.5 rounded-full font-black transition-all whitespace-nowrap">☕ Donar</button>
          ...
          <div className="fixed inset-0 z-50 flex max-w-full overflow-x-hidden">
            ...
            <div className="relative bg-[#0f0f0f] border-r border-zinc-800 w-80 max-w-[85vw] h-full p-6 flex flex-col z-10 overflow-y-auto space-y-4">
              ...
              <div className="flex flex-col space-y-2 text-sm font-semibold">
                ...
                <button onClick={() => { setShowDonateModal(true); setShowMenu(false); }} className="flex items-center gap-3 p-3 rounded-2xl bg--500/10 text--400 border border--500/20 font-bold">☕ Apóyame con una Donación</button>
                ...
                <div className="pt-2">
                  <button onClick={() => { setShowMenu(false); setShowAdminModal(true); }} className="w-full py-3 rounded-2xl bg--500 text-black font-black text-center hover:bg--400">+ Subir Video (Admin)</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="px-3 pt-3 pb-2 w-full max-w-full overflow-x-hidden box-border">
          <div className="sm:hidden mb-2.5 w-full">
            <input 
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)} 
              className="w-full bg-[#121212] border border-zinc-800 p-2.5 rounded-xl text-xs focus:border--500 outline-none text-zinc-200 box-border" 
            />
          </div>
          ...
          <div className="flex items-center justify-between gap-2 mb-2 pb-2 border-b border-zinc-800/60 w-full">
            ...
            <div className="flex items-center gap-1 bg-zinc-900 border border-zinc-800 p-1 rounded-xl text-[11px]">
              <button onClick={() => setSortBy('recent')} className={`px-2.5 py-1 rounded-lg font-bold transition-all ${sortBy === 'recent' ? 'bg--500 text-black' : 'text-zinc-400 hover:text-white'}`}>Más Recientes</button>
              <button onClick={() => setSortBy('likes')} className={`px-2.5 py-1 rounded-lg font-bold transition-all ${sortBy === 'likes' ? 'bg--500 text-black' : 'text-zinc-400 hover:text-white'}`}>Más Gustados</button>
            </div>
          </div>

          <div className="flex gap-1.5 overflow-x-auto pb-1 no-scrollbar w-full max-w-full">
            {defaultTags.map(tag => (
              <button key={tag} onClick={() => setActiveTag(tag)} className={`px-3 py-1 rounded-lg text-[11px] font-bold whitespace-nowrap transition-colors flex-shrink-0 ${activeTag === tag ? 'bg--500 text-black' : 'bg-zinc-800 text-zinc-300 hover:bg-zinc-700'}`}>
                {tag}
              </button>
            ))}
          </div>
        ...
        <section className="px-3 py-4 w-full max-w-full overflow-x-hidden box-border">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-black text--500 tracking-wider uppercase flex items-center gap-1.5">
        ...
        <section className="px-3 pb-12 pt-2 w-full max-w-full overflow-x-hidden box-border">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 w-full max-w-full">
              ...
              <div className="flex flex-col rounded-2xl bg-zinc-900/95 border border--500/40 p-2 shadow-xl min-h-[280px] w-full max-w-full overflow-hidden box-border">
                <div className="flex justify-between items-center mb-1 px-1">
                  <span className="text-[10px] font-bold text--500 uppercase tracking-wider">Patrocinado</span>
                    ...
                    <div onClick={() => handleSelectVideo(video)} className="group cursor-pointer flex flex-col h-full relative w-full">
                      <div className="relative aspect-video rounded-xl overflow-hidden bg-zinc-900 border border-zinc-800 w-full">
                        ...
                        <button 
                          ...
                          className={`absolute top-2 right-2 p-1.5 rounded-full backdrop-blur-md transition-all ${isSaved ? 'bg--500 text-black' : 'bg-black/60 text-white hover:bg-black'}`}
                      ...
                      <div className="mt-2.5 flex gap-2.5 w-full">
                        <div className="w-7 h-7 rounded-full bg--500 text-black font-black flex items-center justify-center flex-shrink-0 text-[11px]">
                          F
                        </div>
        ...
        {selectedVideo && isPipActive && (
          <div className="fixed bottom-4 right-4 z-50 w-80 max-w-[90vw] bg-zinc-950 border border--500/50 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
                    ...
                    <div key={v.id} className="bg-zinc-900 p-2 rounded-xl flex gap-3 items-center border border-zinc-800">
                      ...
                      <div className="flex-1 overflow-hidden">
                        ...
                        <div className="flex gap-2 mt-2">
                          <button onClick={() => { handleSelectVideo(v); setShowWatchLaterModal(false); }} className="text-[10px] bg--500 text-black px-2 py-1 rounded font-bold">Ver</button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setShowDonateModal(false)}>
            <div className="bg-zinc-950 border border-zinc-800 p-6 rounded-3xl max-w-md w-full space-y-4 text-center" onClick={e => e.stopPropagation()}>
              ...
              <a 
                ...
                className="block w-full bg--500 hover:bg--400 text-black font-black py-3.5 rounded-xl text-sm transition-all"
              >
                Donar con PayPal
              </a>
            ...
            <div className="max-w-4xl mx-auto">
              <div className="flex justify-between items-center mb-8 pb-4 border-b border-zinc-900">
                ...
                <div className="flex gap-3 items-center">
                  <button onClick={() => setShowAdminProd(true)} className="text-xs bg-zinc-900 hover:border--500 border border-zinc-800 text-zinc-200 px-4 py-2 rounded-xl font-bold">+ Agregar Producto</button>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
                {products.map(p => (
                  <a key={p.id} href={p.buy_url} target="_blank" rel="noopener noreferrer" className="bg-zinc-900/60 p-3 rounded-2xl border border-zinc-800 hover:border--500 transition-all flex flex-col group">
                    ...
                    <div className="mt-2 flex items-center justify-between">
                      <span className="text--500 font-black text-sm">{p.price}</span>
                      <span className="text-[10px] bg--500/10 text--500 px-2 py-1 rounded-md font-bold">Ver en Amazon</span>
                    </div>
                  </a>
                ))}
                {products.length === 0 && (
        ...
        {showAdminProd && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
            <form onSubmit={handleSaveProduct} className="bg-zinc-950 p-6 rounded-3xl border border-zinc-800 w-full max-w-md space-y-4">
              ...
              <input type="password" placeholder="Clave de administrador" value={adminPassword} onChange={e => setAdminPassword(e.target.value)} className="w-full bg-zinc-900 p-3 rounded-xl border border-zinc-800 text-white outline-none focus:border--500" />
              <input type="text" placeholder="Nombre del producto" value={prodTitle} onChange={e => setProdTitle(e.target.value)} className="w-full bg-zinc-900 p-3 rounded-xl border border-zinc-800 text-white outline-none focus:border--500" />
              <input type="text" placeholder="Precio (ej. $19.99 USD)" value={prodPrice} onChange={e => setProdPrice(e.target.value)} className="w-full bg-zinc-900 p-3 rounded-xl border border-zinc-800 text-white outline-none focus:border--500" />
              <input type="text" placeholder="URL de la imagen del producto" value={prodImage} onChange={e => setProdImage(e.target.value)} className="w-full bg-zinc-900 p-3 rounded-xl border border-zinc-800 text-white outline-none focus:border--500" />
              <input type="text" placeholder="Enlace de afiliado (ej. https://amzn.to/...)" value={prodBuyUrl} onChange={e => setProdBuyUrl(e.target.value)} className="w-full bg-zinc-900 p-3 rounded-xl border border-zinc-800 text-white outline-none focus:border--500" />
              <div className="flex gap-2 pt-2">
                ...
                <button type="submit" className="w-full p-3 rounded-xl bg--500 text-black font-black hover:bg--400">Guardar Afiliado</button>
              </div>
            </form>
          </div>
        )}
            ...
            <div id="video-modal-container" className={`bg-[#0f0f0f] w-full min-h-screen md:min-h-0 ${isCinemaMode ? 'md:max-w-6xl' : 'md:max-w-4xl'} md:rounded-3xl overflow-hidden flex flex-col my-auto border border-zinc-800 transition-all duration-300`} onClick={e => e.stopPropagation()}>
              ...
              <div className="bg-zinc-950 px-4 py-2 border-b border-zinc-800 flex justify-between items-center text-xs">
                <div className="flex items-center gap-2">
                  <button onClick={() => setIsCinemaMode(!isCinemaMode)} className={`px-2.5 py-1 rounded-lg font-bold border ${isCinemaMode ? 'bg--500 text-black border--500' : 'bg-zinc-900 text-zinc-300 border-zinc-800 hover:bg-zinc-800'}`}>
                    🎬 Modo Cine
                  </button>
                ...
                <div className="flex items-center gap-3">
                  <label className="flex items-center gap-1.5 cursor-pointer text-zinc-400 font-semibold select-none">
                    <input type="checkbox" checked={autoPlayNext} onChange={(e) => setAutoPlayNext(e.target.checked)} className="accent--500" />
                    Autoplay Siguiente
                  </label>
                  <button onClick={handleNextVideo} className="bg--500 text-black font-black px-3 py-1 rounded-lg hover:bg--400">
                    Siguiente ➔
                  </button>
                </div>
              </div>
              ...
              <div className="p-4 bg-[#0f0f0f] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-zinc-800">
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto justify-end flex-wrap">
                  <button 
                    ...
                    className={`flex items-center gap-1.5 text-xs px-3.5 py-2 rounded-full font-bold transition-colors ${userLikedMap[selectedVideo.id] ? 'bg--500 text-black' : 'bg-zinc-800 hover:bg-zinc-700 text-zinc-200'}`}
                  >
                  ...
                  <button 
                    ...
                    className={`text-xs px-3.5 py-2 rounded-full font-bold border transition-colors ${watchLater.some(v => v.id === selectedVideo.id) ? 'bg--500 text-black border--500' : 'bg-zinc-800 hover:bg-zinc-700 text-zinc-200 border-zinc-700'}`}
                  >
                  ...
                  <button 
                    ...
                    className="bg--500 text-black hover:bg--400 text-xs px-4 py-2 rounded-full font-black"
                  >
                    ☕ Donar
                  </button>
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {(Array.isArray(selectedVideo.tags) ? selectedVideo.tags : [selectedVideo.category, 'HD']).map(t => (
                    <button 
                      ...
                      className="bg-zinc-800 hover:bg--500 hover:text-black text--400 text-[11px] font-bold px-3 py-1 rounded-full transition-colors"
                    >
                      #{t}
                    </button>
                  ))}
                ...
                <form onSubmit={(e) => handleAddComment(selectedVideo.id, e)} className="space-y-2">
                  <div className="flex gap-2">
                    <input 
                      ...
                      className="w-1/3 bg-zinc-950 border border-zinc-800 px-3 py-2 rounded-xl text-xs text-white outline-none focus:border--500" 
                    />
                  </div>
                  <div className="flex gap-2">
                    <input 
                      ...
                      className="flex-grow bg-zinc-950 border border-zinc-800 px-3 py-2 rounded-xl text-xs text-white outline-none focus:border--500" 
                    />
                    <button type="submit" className="bg--500 text-black font-bold px-4 py-2 rounded-xl text-xs hover:bg--400">Comentar</button>
                  </div>
                </form>
                  ...
                  {(commentsMap[selectedVideo.id] || commentsMap['default']).map(c => (
                    <div key={c.id} className="bg-zinc-950/60 p-3 rounded-2xl border border-zinc-800/60 text-xs space-y-1">
                      <div className="flex justify-between items-center">
                        <span className="font-bold text--500">{c.user}</span>
              ...
              <div className="p-4 bg-[#0d0d0d] border-t border-zinc-800 space-y-3">
                <h3 className="text-xs font-black text--500 uppercase tracking-wider flex items-center gap-1.5">
        ...
        {showAdminModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
            <form onSubmit={handleSaveVideo} className="bg-zinc-950 p-6 rounded-3xl border border-zinc-800 w-full max-w-md space-y-4 max-h-[90vh] overflow-y-auto">
              ...
              <input type="password" placeholder="Clave de administrador" value={adminPassword} onChange={e => setAdminPassword(e.target.value)} className="w-full bg-zinc-900 p-3 rounded-xl border border-zinc-800 text-white outline-none focus:border--500" />
              <input type="text" placeholder="Título del video" value={title} onChange={e => setTitle(e.target.value)} className="w-full bg-zinc-900 p-3 rounded-xl border border-zinc-800 text-white outline-none focus:border--500" />
              <select value={category} onChange={e => setCategory(e.target.value)} className="w-full bg-zinc-900 p-3 rounded-xl border border-zinc-800 text-zinc-300 outline-none focus:border--500">
                {defaultTags.filter(t => t !== 'Todos').map(t => <option key={t} value={t}>{t}</option>)}
              </select>
              <textarea placeholder="Descripción del video personalizada" value={description} onChange={e => setDescription(e.target.value)} rows={3} className="w-full bg-zinc-900 p-3 rounded-xl border border-zinc-800 text-white outline-none focus:border--500 resize-none" />
              ...
              <div className="flex items-center gap-3 bg-zinc-900 p-3 rounded-xl border border-zinc-800">
                <input 
                  ...
                  className="w-4 h-4 accent--500 cursor-pointer" 
                />
              ...
              <input type="text" placeholder="Etiquetas (separadas por coma: HD, Rubias, etc.)" value={videoTagsInput} onChange={e => setVideoTagsInput(e.target.value)} className="w-full bg-zinc-900 p-3 rounded-xl border border-zinc-800 text-white outline-none focus:border--500" />
              <input type="text" placeholder="URL StreamHG / Embed del video" value={voeUrl} onChange={e => setVoeUrl(e.target.value)} className="w-full bg-zinc-900 p-3 rounded-xl border border-zinc-800 text-white outline-none focus:border--500" />
              <input type="text" placeholder="URL Portada / Miniatura" value={coverUrl} onChange={e => setCoverUrl(e.target.value)} className="w-full bg-zinc-900 p-3 rounded-xl border border-zinc-800 text-white outline-none focus:border--500" />
              <div className="flex gap-2 pt-2">
                ...
                <button type="submit" className="w-full p-3 rounded-xl bg--500 text-black font-black hover:bg--400">Publicar</button>
              </div>
            </form>
          </div>
        )}
      ...
      <footer className="bg-black border-t border-zinc-900 py-10 px-4 mt-12 text-center text-xs text-zinc-500 space-y-6 w-full max-w-full overflow-x-hidden box-border">
        ...
        <div className="flex justify-center gap-6 font-semibold text-zinc-400 flex-wrap">
          <a href="#" className="hover:text--500">Política Y privacidad</a>
          <span>•</span>
          <a href="#" className="hover:text--500">DMCA</a>
          <span>•</span>
          <a href="#" className="hover:text--500">2257</a>
          <span>•</span>
          <a href="mailto:umbrellaholdings.global@gmail.com" className="hover:text--500">Contacto</a>
        </div>
