...
const vShorts = videos.filter(v => v.is_short);
if (vShorts.length > 0) {
const nextIdx = (activeShortIndex + 1) % vShorts.length;
setActiveShortIndex(nextIdx);
handleSelectVideo(vShorts[nextIdx]);
}
} else {
...
const handlePrevVideoForShorts = () => {
const vShorts = videos.filter(v => v.is_short);
if (vShorts.length === 0) return;
const prevIdx = (activeShortIndex - 1 + vShorts.length) % vShorts.length;
setActiveShortIndex(prevIdx);
handleSelectVideo(vShorts[prevIdx]);
};
...
<span className="text-white text-xs font-black uppercase tracking-widest bg-black/40 px-3 py-1 rounded-full border border-zinc-800">
⚡ Shorts TikTok Feed
</span>
<button onClick={(e) => handleShare(selectedVideo, e)} className="text-white bg-zinc-900/60 p-2 rounded-full backdrop-blur-md">
🔗
</button>
</div>
...
{(selectedVideo.author || 'F').charAt(0).toUpperCase()}
</div>
<span className="text-white text-xs font-bold drop-shadow">@{selectedVideo.author || 'FlixxesUser'}</span>
</div>
...
>
<div className="p-3 rounded-full bg-black/60 backdrop-blur-md hover:bg-black transition-all">
💬
</div>
...
className="pointer-events-auto bg-zinc-900/80 hover:bg-zinc-800 text-white text-xs font-black px-4 py-2 rounded-full backdrop-blur-md border border-zinc-700 shadow"
>
▲ Anterior
</button>
<button 
onClick={handleNextVideo} 
className="pointer-events-auto bg-blue-600 hover:bg-blue-500 text-white text-xs font-black px-6 py-2.5 rounded-full shadow-lg"
>
Siguiente Short ➔
</button>
</div>
...
<h3 className="text-white text-sm font-black">Comentarios</h3>
<button onClick={() => setShowCommentsDrawer(false)} className="text-zinc-400 hover:text-white text-xs font-bold">✕ Cerrar</button>
</div>
<div className="flex-1 overflow-y-auto space-y-3 pr-1">
{(commentsMap[selectedVideo.id] || commentsMap['default']).map(c => (
<div key={c.id} className="bg-zinc-900/80 p-3 rounded-2xl border border-zinc-800 text-xs space-y-1">
<div className="flex justify-between items-center">
...
<form onSubmit={(e) => handleAddComment(selectedVideo.id, e)} className="flex gap-2 pt-2 border-t border-zinc-800">
<input
type="text"
placeholder="Añade un comentario..."
value={newCommentText}
onChange={(e) => setNewCommentText(e.target.value)}
className="flex-1 bg-zinc-900 border border-zinc-800 px-3 py-2.5 rounded-xl text-xs text-white outline-none focus:border-blue-500"
/>
<button type="submit" className="bg-blue-600 hover:bg-blue-500 text-white font-bold px-4 py-2.5 rounded-xl text-xs">Comentar</button>
</form>
</div>
</div>
)}
